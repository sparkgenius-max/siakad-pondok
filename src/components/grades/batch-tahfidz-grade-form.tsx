...
import { saveBatchTahfidzGrades } from '@/app/(dashboard)/grades/actions'
...
interface BatchTahfidzGradeFormProps {
...
export function BatchTahfidzGradeForm({
    santriList,
    existingGrades,
    subject,
    semester,
    ...
    const [grades, setGrades] = useState<Record<string, GradeEntry>>(() => {
        ...
        santriList.forEach(santri => {
            ...
            initial[santri.id] = {
                santri_id: santri.id,
                score_total: existing?.score_total?.toString() || '',
    ...
    const updateScore = (santriId: string, field: 'score_total' | 'notes', value: string) => {
        setGrades(prev => ({
            ...prev,
    ...
    const handleSubmit = async () => {
        ...
        const result = await saveBatchTahfidzGrades({
            grades: gradesToSave,
            subject,
            semester,
    ...
    const getGradeColor = (scoreStr: string) => {
        ...
        if (!scoreStr || isNaN(score)) return 'text-slate-300'
    ...
    return (
        <Card className="border-none shadow-md overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between border-b bg-slate-50/50 py-4">
                ...
                <Button onClick={handleSubmit} disabled={saving} className="bg-cyan-600 hover:bg-cyan-700 h-10 px-6">
                    {saving ? (
                    ...
                    <Table>
                        <TableHeader className="bg-slate-50/80">
                            <TableRow>
                                ...
                                <TableHead className="w-[120px] text-center font-bold">Nilai</TableHead>
                        ...
                        <TableBody>
                            {santriList.map((santri, index) => {
                                ...
                                const hasValue = entry?.score_total && entry.score_total.trim() !== ''
                                    ...
                                    <TableRow key={santri.id} className="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <TableCell>
                                            <Input
                                                ...
                                                max="100"
                                                placeholder="0"
                                                value={entry?.score_total || ''}
                                                onChange={(e) => updateScore(santri.id, 'score_total', e.target.value)}
                                                className={`text-center h-9 focus-visible:ring-cyan-500 bg-white border-slate-200 font-bold ${getGradeColor(entry?.score_total)}`}
                                            />
                                        </TableCell>
                                        <TableCell>
                                            <Input
                                                ...
                                                className="h-9 focus-visible:ring-cyan-500 bg-white border-slate-200"
                                            />
                                        </TableCell>
