...
import { ArrowLeft, BookOpen, Users, Save } from 'lucide-react'
import Link from 'next/link'
import { BatchGradeForm } from '@/components/grades/batch-grade-form'
...
export default async function BatchGradePage({
    ...
    const buildUrl = (newParams: Record<string, string>) => {
        ...
        const merged = { class: params.class, subject: params.subject, semester: params.semester, year: params.year, ...newParams }
        Object.entries(merged).forEach(([k, v]) => {
            if (v) urlParams.set(k, v)
        })
        return `/grades/batch?${urlParams.toString()}`
    }
    ...
    return (
        <div className="flex-1 space-y-6 p-8 pt-6">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-3xl font-bold tracking-tight">Input Nilai Batch</h2>
                    <p className="text-muted-foreground">Input nilai sekaligus per kelas dan mata pelajaran</p>
                </div>
            </div>
